...
namespace FFPL {
	...
	class Utils extends Crypt {
		 ...
		 * @return boolean Returns true if the file is an image, false if not, or an error occur
		 * */
		...
		/**
		 * Erase one file from the file system
		 *
		...
		/**
		 * Erase a lot of files from the same path
		 *
		 * @var string The source path
		 * @var array An array that contains all filenames to erase
		 * @return boolean Return true if erase is ok, false if any error occur
		 * */
		public static function eraseFiles($path, array $fileNames) {
			foreach ($fileNames as $fileName) :
				if (!$this -> eraseFile($path, $fileName))
					return false;
			endforeach;
			return true;
		}
		...
		/**
		 * Convert any POST|GET|FILES(prefered FILES) array to sequential vector
		 *
		 * @var array POST|GET|FILES input
		...
		/**
		 * This functions will add a watermark to an given image ( not fully implemented)
		 *
		 * @var string Full path to image file
		 * @var string Full path to overlay image file(need be a png file)
		 * @var array Optional parameters to increase the output performance
		 * @return boolean Return true if the image with watermark has been generated,
		 * false if any error occur
		 * */
		private function overlay_watermark($fullPathImagem, $fullPathOverlay, $options = array('opacity'=>20,'output_quality'=>100,'overwrite'=>true,'output_path'=>'')) {
		...
		/**
		 * This function will encode the key password from a database string encode to be
		...
		/**
		 * This function will decode the string generated on the function encodeKey().
		 * This is a implementation of the abstract class FFPL\Crypt
		 *
