...
namespace FFPL {
	...
	class Utils extends Crypt {
		 ...
		 * @return boolean Returns true if the file is an image, false if not, or an
		 * error occur
		 * */
		...
		public function overlayPDF($fullPathPDF, $fullPathOverlayPNG) {
			$fpdi = new \FPDI;
			$fpdi -> addPage();
			$fpdi -> numPages = $fpdi -> setSourceFile($fullPathPDF);
			$fpdi -> _tplIdx = $fpdi -> importPage(1);
			$width = $fpdi -> w;
			$height = $fpdi -> h;
			...
			$h = 0;
			$m = 0;
			$numpages = $fpdi -> numPages;
			$i = 1;
			if ($fpdi -> numPages > 1) :
				while ($i <= $numpages) :
					for ($j = 0; $j < 5; $j++) :
						for ($k = 0; $k < 9; $k++) :
							$fpdi -> Image($fullPathOverlayPNG, $m, $h, $wm_width, $wm_height);
							$h = $h + $wm_height;
						endfor;
						$m = $m + $wm_width;
						$h = 0;
